...
class RAGService:
    ...
    async def query(self, query: str, top_k: int = 3) -> dict:
        ...
        initial_state = self._new_state(query=query, top_k=top_k)
    ...
    async def query_stream(
        ...
        state = self._new_state(query=query, top_k=top_k)
    ...
    async def _tool_orchestrator(self, state: QueryState) -> Dict[str, Any]:
        ...
        for step in steps:
            ...
            try:
                ...
                elif tool in {"llm_reason", "sql_query", "web_fetch", "calculator"}:
                    tool_results.append(
                        {
                            "step_id": step.get("id"),
                            "tool": tool,
    ...
    async def _decision_node(self, state: QueryState) -> Dict[str, Any]:
        ...
        return {"retries_used": retries_used + 1}

    async def _finalize_response(self, state: QueryState) -> Dict[str, Any]:
        if state.get("validator_decision") == "fail_safe":
            return {"final_answer": self._build_fail_safe_answer(state)}
    ...
    async def _call_structured_with_retry(self, fn, *args, **kwargs) -> dict[str, Any]:
